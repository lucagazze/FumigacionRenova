...
// --- Toda la lógica se envuelve en este evento para esperar a que el HTML esté listo ---
document.addEventListener('DOMContentLoaded', () => {

    // --- Elementos del DOM (se obtienen aquí para asegurar que existen) ---
    ...
    async function poblarFiltros() {
        ...
        depositos.forEach(d => {
            if (d.clientes) { // Asegurarse que el cliente no sea null
                filtroDeposito.innerHTML += `<option value="${d.id}">${d.nombre} (${d.tipo}) - ${d.clientes.nombre}</option>`;
            }
        });
    }
