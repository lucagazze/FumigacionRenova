...
async function renderFinishedOperations() {
    ...
    container.innerHTML = '<p class="text-center p-4 text-gray-500 col-span-full">Cargando historial...</p>';
    ...
    if (limpiezasError || finalesError) {
        container.innerHTML = '<p class="text-red-500 text-center p-4 col-span-full">Error al cargar datos auxiliares.</p>';
        return;
    }

    // Mapeamos los datos para búsqueda rápida
    ...
    // 2. Obtener las operaciones finalizadas aplicando filtros de fecha.
    let query = supabase
        .from('operaciones')
    ...
    // 3. Filtrar y ordenar los datos obtenidos
    ...
    container.innerHTML = filteredOps.map(op => {
        ...
        // ===== LÓGICA DE GARANTÍA CALCULADA "AL VUELO" =====
        let garantiaHtml = '';
        const registroFinal = todosLosFinales.find(f => f.operacion_original_id === op.id);

        if (registroFinal) {
            const fechaFinDate = new Date(registroFinal.created_at);
            const duracionDias = (fechaFinDate - new Date(op.created_at)) / (1000 * 60 * 60 * 24);
            ...
            if (fechaGarantiaLimpieza) {
                if (fechaFinDate <= new Date(fechaGarantiaLimpieza + 'T00:00:00')) {
                    cumpleLimpieza = true;
                }
            }
            ...
            if (cumplePlazo && cumpleLimpieza) {
                ...
                const fechaVencimientoGarantia = new Date(fechaFinDate);
                fechaVencimientoGarantia.setDate(fechaVencimientoGarantia.getDate() + 40);
                const vencimientoStr = fechaVencimientoGarantia.toLocaleDateString('es-AR');
                ...
                garantiaHtml = `<div class="flex items-center gap-2" title="No cumplió los requisitos para garantía"><span class="material-icons text-lg text-red-600">cancel</span><span>Garantía: <strong class="text-red-700">No incluida</strong></span></div>`;
            }
        } else {
             garantiaHtml = `<div class="flex items-center gap-2" title="Sin registro de finalización"><span class="material-icons text-lg text-gray-500">help_outline</span><span>Garantía: <strong class="text-gray-600">N/A</strong></span></div>`;
        }
